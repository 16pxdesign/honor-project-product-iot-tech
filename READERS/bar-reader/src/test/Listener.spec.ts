...
import {Client, MaximumListenerExceeded, ReaderHandler, UniqueListenerExist} from "../ReaderHandler";
...
describe('Delete user', () => {
    ...
    it('test validation function',  async() => {
        ...
        expect(err1.length).to.equal(0)
        await ReaderHandler.getInstance().setLister(eventEmitter, 'on', () => {        }, 'cli', '1');
        let err2 = await ReaderHandler.getInstance().validation(eventEmitter,'xx','0');
        //console.log(err2)
        expect(err2[0]).to.be.an.instanceof(MaximumListenerExceeded);
