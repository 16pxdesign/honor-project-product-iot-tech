...
import HIDES, {dev} from "./DeviceWrapper";
...
export class Reader {
    ...
    device: any | undefined = undefined;
    ...
    async init(): Promise<boolean> {
        return new Promise( (resolve, reject) =>{
            try{
                this.device = HIDES.open((error: any)=>{
                    throw error;
                });
            }catch (error) {
                if (error) {
    ...
    private async onStartScan(value: any) {
        ...
        //create listener
        const listener = await ReaderHandler.getInstance().setValidatedListener(this.device, 'data', callback, value.client,value.transaction,value.timeout);
        if(!listener)
    ...
    private onStopScan(value: any) {
        //TODO: TEST IT?
        ReaderHandler.getInstance().deleteListener(this.device,value.client,'data',value.transaction)
